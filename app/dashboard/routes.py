...
def customers():
    ...
    customers_data = customers_query.order_by(desc('total_spent')).all()
    ...
    new_customers_count = len([c for c in filtered_customers if c['type'] == 'new'])
    
    # Calcular tasa de retorno
    total_customers = len(filtered_customers)
    returning_customers = len([c for c in filtered_customers if c['total_orders'] > 1])
